...
class Main:
    def __init__(self, master):
        ...
        # 기본 창 설정
        self.master.title("한컴타자연습")  # 창의 제목 설정
        self.master.geometry("1024x768")  # 창의 크기 설정
        self.master.configure(fg_color="#AAAAAA")  # 창의 배경색 설정
        ...
        # 메인 프레임 설정
        frame = ctk.CTkFrame(master=self.master, width=1024, height=728, fg_color="#AAAAAA")
        frame.pack(pady=10, padx=10, fill="both", expand=True)
        frame.pack(pady=10, padx=5, fill="both", expand=True)

        # 메인 메뉴 초기화 및 생성
        MainMenu(root=self.master, master=frame)
        ...
        image_label = TKLabel(parent_frame, text=None).create_label(image=self.photo, fg_color="white")
