...
class LongTextWindow:
    ...
    def display_text_with_labels(self):
        ...
        self.page_text = self.text_content[start_index:start_index + end_index]
    ...
    def next_page(self):
        print(self.current_page, len(self.text_content))
        if self.current_page < len(self.text_content) // self.lines_per_page:
            self.current_page += 1
    ...
    def on_keyrelease(self, text, event):
        ...
        if len(self.page_text) == 0 or len(self.page_text) <= self.current_label_index:
            self.next_page()
            return False
        ...
        if self.current_label_index < len(self.page_text):
            ...
            original_text = self.labels[self.current_label_index][0].cget("text")
            input_text = self.labels[self.current_label_index][1].cget("text")
            if event != "Return":
                input_text = input_text[:len(original_text)]

            accuracy, matches, mismatches, typing_speed = self.measure_manager.calculate(original_text, input_text)

            # if accuracy != None:
            self.measure_manager.append_result(accuracy, matches, mismatches, typing_speed)
    ...
    def on_text_changed(self, text):
        ...
        self.measure_manager.calculate(original_text, input_text)
