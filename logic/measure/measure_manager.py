...
class MeasureManager:
    ...
    def updateSpeed(self):
        if self.start_time is not None:
            ...
            num_chars = self.total_len
            for index, i in enumerate(self.input_field_text):
                if(self.input_field_text[index] == i):
                    num_chars += 1
            self.num_chars = num_chars
            self.current_cpm = int((num_chars / self.elapsed_time) * 60)
    ...
    def calculate_overall_accuracy(self, input_fields, texts):
        for input_field, base_text in zip(input_fields, texts):
            ...
            print("asdasd : ", self.total_mismatched_characters, self.total_characters)
